...
export function defineRoutes(controllers: any, application: Express) {
    for (let i = 0; i < controllers.length; i++) {
        ...
        const conrtollerPath: string = Reflect.getMetadata('baseRoute', controller.constructor);
        ...
        for (let j = 0; j < methods.length; j++) {
            ...
            if (routes) {
                ...
                for (let k = 0; k < routeNames.length; k++) {
                    ...
                    if (handlers) {
                        application[method](conrtollerPath + routeNames[k], handlers);
                        logging.log(`Route ${method.toString()} ${conrtollerPath + routeNames[k]} loading...`);
                    }
                }
            }
        }
